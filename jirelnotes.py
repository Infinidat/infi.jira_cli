...
def render_release_notes(project_key, include_next_release, page_id, header_id, footer_id):
    ...
    next_release_name = get_next_release_name_in_project(project_key)
    
    real_versions = [version for version in reversed(project.versions) if
                     match(r'[0-9\.]+', version.name) and not version.archived and version.released]
    
    if include_next_release:
        next_release = next((version for version in project.versions if version.name == next_release_name), None)
        if next_release:
            real_versions.insert(0, next_release)
